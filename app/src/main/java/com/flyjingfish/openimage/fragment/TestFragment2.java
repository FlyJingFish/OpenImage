...
public class TestFragment2 extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.llRoot.rv.rv.setLayoutManager(new LinearLayoutManager(getActivity()));
        loadData();
        setSelect(0);
        ...
        binding.llRoot.btnH.setOnClickListener(v -> {
            setSelect(1);
            binding.llRoot.rv.rv.setLayoutManager(new LinearLayoutManager(getActivity(), LinearLayoutManager.HORIZONTAL, false));
            loadData();
        });
        binding.llRoot.btnG.setOnClickListener(v -> {
            setSelect(2);
            binding.llRoot.rv.rv.setLayoutManager(new GridLayoutManager(getActivity(), 2));
            loadData();
        });
        ...
        binding.llRoot.btnC.setOnClickListener(v -> {
            setSelect(4);
            customLayoutManager = new FlexboxLayoutManager(getActivity()){
                @Override
    ...
    private void loadData() {
        MyApplication.cThreadPool.submit(() -> {
            ...
            String response1 = DataUtils.getFromAssets(getActivity(), "listview_data.json");
            try {
    ...
    private void setData(List<ImageEntity> datas) {
        getActivity().runOnUiThread(() -> binding.llRoot.rv.rv.setAdapter(new RvAdapter(datas)));
    ...
    private class RvAdapter extends RecyclerView.Adapter<RvAdapter.MyHolder> {
        ...
        public void onBindViewHolder(@NonNull MyHolder holder, int position) {
            if (layoutType == 4) {
                ...
                layoutParams.width = (int) ScreenUtils.dp2px(getActivity(), 86 * randoms[position]);
                holder.ivImage.setLayoutParams(layoutParams);
            }else if (layoutType == 1) {
                ViewGroup.LayoutParams layoutParams = holder.ivImage.getLayoutParams();
                layoutParams.width = (int) ScreenUtils.dp2px(getActivity(), 100);
                layoutParams.height = (int) ScreenUtils.dp2px(getActivity(), 100);
                holder.ivImage.setLayoutParams(layoutParams);
            } else if (layoutType == 0 || layoutType == 2) {
                ViewGroup.LayoutParams layoutParams = holder.ivImage.getLayoutParams();
                layoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT;
                layoutParams.height = (int) ScreenUtils.dp2px(getActivity(), 220);
                holder.ivImage.setLayoutParams(layoutParams);
            }
            ...
            holder.ivImage.setOnClickListener(v -> {
                OpenImage openImage = OpenImage.with(TestFragment2.this);
                if (layoutType == 4){
                ...
                openImage.setAutoScrollScanPosition(true)
                        ...
                        .setShowDownload()
                        .setOnExitListener(() -> Toast.makeText(getActivity(),"onExit",Toast.LENGTH_SHORT).show())
